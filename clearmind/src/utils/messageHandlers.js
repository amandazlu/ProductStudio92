...
import { 
  handleEventCreatedWithNotification,
  handleEventUpdatedWithNotification,
  handleEventDeletedWithNotification 
} from './familyNotifications.js';
...
export async function handleMultipleEventsCreation(
  ...
  try {
    ...
    // Notify family groups about each created event
    for (const createdEvent of createdEventsList) {
      // Send notifications to family groups
      await handleEventCreatedWithNotification(
        userEmail,
        createdEvent,
        false // Set to true to prompt user for which groups to notify
      );
    }
...
export async function handleEventDeletion(
  ...
  try {
    ...
    // Send notifications to family groups about the deletion
    await handleEventDeletedWithNotification(
      userEmail,
      event,
      false
    );
...
export async function handleEventUpdate(
  ...
  try {      
    ...
    // Send notifications to family groups about the update
    await handleEventUpdatedWithNotification(
      userEmail,
      updatedEvent,
      { 
        oldTime: eventToUpdate.start,
        newTime: updatedEvent.start 
      },
      false
    );
